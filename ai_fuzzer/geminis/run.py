...
def retrieve_function_candidates(path: Path, prompt_id: str, prompt_yaml_path: Path, api: str, debug: False) -> Sequence[str]:
    func_tests = []
...
def retrieve_class_candidates(path: Path, prompt_id: str, prompt_yaml_path: Path, api: str, debug: False) -> Sequence[str]:
    class_tests = []
    pyfiles = function_parser.get_python_file_paths(path, debug=debug)
    if debug:
        print(f"DEBUG: Retrieved {len(pyfiles)} Python files from: {path}")
...
def run(
    source_dir: Path, output_dir: Path, prompt_id: str, mode: str, prompt_yaml_path: Path, api: str, debug: False
) -> None:
    if debug:
    ...
    if mode == "functions":
        code_snippets = retrieve_function_candidates(source_dir, prompt_id, prompt_yaml_path, api=api, debug=debug)
    elif mode == "classes":
        code_snippets = retrieve_class_candidates(source_dir, prompt_id, prompt_yaml_path, api=api, debug=debug)
    else:
